...
$model = new app;
$others = new another();
$idNss = (isset($_GET['nss']) ? $_GET['nss'] : null);
$jason =  $model->getDatsNss($idNss);
...
foreach ($decoded->Datos as $key) {
    ...
    $CARDIOPATIA_ISQUEMICA = ($key->Data->CARDIOPATIA->CARDIOPATIA_ISQUEMICA == 0) ? "No" : "Sí";
    $HIPERTENSION = ($key->Data->CARDIOPATIA->HIPERTENSION == 0) ? "No" : "Sí";
    $HIPOTENSION = ($key->Data->CARDIOPATIA->HIPOTENSION == 0) ? "No" : "Sí";
    $INSUFICIENCIA_CARDIACA = ($key->Data->CARDIOPATIA->INSUFICIENCIA_CARDIACA == 0) ? "No" : "Sí";
    $INSUFICIENCIA_VASCULAR_PERIFERICA = ($key->Data->CARDIOPATIA->INSUFICIENCIA_VASCULAR_PERIFERICA == 0) ? "No" : "Sí";
    ...
    $BRONQUITIS = ($key->Data->NEUMOPATICA->BRONQUITIS == 0) ? "No" : "Sí";
    $TUBERCULOSIS = ($key->Data->NEUMOPATICA->TUBERCULOSIS == 0) ? "No" : "Sí";
    $ASMA = ($key->Data->NEUMOPATICA->ASMA == 0) ? "No" : "Sí";
    $INSUFICIENCIA_RESPIRATORIA = ($key->Data->NEUMOPATICA->INSUFICIENCIA_RESPIRATORIA == 0) ? "No" : "Sí";
    ...
    $ALCOHOLISMO = ($key->Data->TOXICOLOGIOCAS->ALCOHOLISMO == 0) ? "No" : "Sí";
    $TABAQUISMO = ($key->Data->TOXICOLOGIOCAS->TABAQUISMO == 0) ? "No" : "Sí";
    $DROGAS = ($key->Data->TOXICOLOGIOCAS->DROGAS == 0) ? "No" : "Sí";
    ...
    $IRC = ($key->Data->NEFROPATIA->IRC == 0) ? "No" : "Sí";
    $LITASIS = ($key->Data->NEFROPATIA->LITASIS == 0) ? "No" : "Sí";
    ...
    $DM = ($key->Data->ENDOCRINA->DM == 0) ? "No" : "Sí";
    $HIPERTIROIDISMO = ($key->Data->ENDOCRINA->HIPERTIROIDISMO == 0) ? "No" : "Sí";
    $HIPOTIROIDISMO = ($key->Data->ENDOCRINA->HIPOTIROIDISMO == 0) ? "No" : "Sí";
    ...
    $TRANSTORNO_DE_CRECIMIENTO = ($key->Data->OBESIDAD->TRANSTORNO_DE_CRECIMIENTO == 0) ? "No" : "Sí";
    /* Mentales */
    ...
    $NEUROSIS = ($key->Data->MENTALES->NEUROSIS == 0) ? "No" : "Sí";
    $PSICOSIS = ($key->Data->MENTALES->PSICOSIS == 0) ? "No" : "Sí";
    $INTENTO_DE_SUICIDIO = ($key->Data->MENTALES->INTENTO_DE_SUICIDIO == 0) ? "No" : "Sí";
    $ESQUIZOFRENIA = ($key->Data->MENTALES->ESQUIZOFRENIA == 0) ? "No" : "Sí";
    ...
    $CANCER = ($key->Data->GENERALES->CANCER == 0) ? "No" : "Sí";
    $ALERGIAS = ($key->Data->GENERALES->ALERGIAS == 0) ? "No" : "Sí";
    $MALFORMACIONES = ($key->Data->GENERALES->MALFORMACIONES == 0) ? "No" : "Sí";

    /* Otras */
    $OTRAS = $key->Data->OTRAS->OTRAS;
    $OTRAS_OBS = $key->Data->OTRAS->OTRAS_OBS;

    $OTRAS = ($key->Data->OTRAS->OTRAS == 0) ? "No" : "Sí";
    /* APNP */
    ...
    $AGUA_POTABLE = ($key->Data->SERV_MED_ANT_PATOLOGICOS->AGUA_POTABLE == 0) ? "No" : "Sí";
    $LUZ = ($key->Data->SERV_MED_ANT_PATOLOGICOS->LUZ == 0) ? "No" : "Sí";
    $AIRE_ACONDICIONADO = ($key->Data->SERV_MED_ANT_PATOLOGICOS->AIRE_ACONDICIONADO == 0) ? "No" : "Sí";
    $ALIMENTACION = ($key->Data->SERV_MED_ANT_PATOLOGICOS->ALIMENTACION == 0) ? "No" : "Sí";
    $SUFICIENTE = ($key->Data->SERV_MED_ANT_PATOLOGICOS->SUFICIENTE == 0) ? "No" : "Sí";
    $BALANCEADA = ($key->Data->SERV_MED_ANT_PATOLOGICOS->BALANCEADA == 0) ? "No" : "Sí";
    /* Inmunizaciones */
    ...
    $BLG = ($key->Data->SERV_MED_INMUNIZACIONES->BLG == 0) ? "No" : "Sí";
    $SABIN = ($key->Data->SERV_MED_INMUNIZACIONES->{'SABIN '} == 0) ? "No" : "Sí"; // Ten en cuenta el espacio en 'SABIN '
    $DPT = ($key->Data->SERV_MED_INMUNIZACIONES->DPT == 0) ? "No" : "Sí";
    $HEPATITIS_B = ($key->Data->SERV_MED_INMUNIZACIONES->HEPATITIS_B == 0) ? "No" : "Sí";
    $SR = ($key->Data->SERV_MED_INMUNIZACIONES->SR == 0) ? "No" : "Sí";
    $INFLUENZA = ($key->Data->SERV_MED_INMUNIZACIONES->INFLUENZA == 0) ? "No" : "Sí";
    $TD = ($key->Data->SERV_MED_INMUNIZACIONES->TD == 0) ? "No" : "Sí";
    ...
    $MIOPIA_OBS = $key->Data->SERV_MED_OFTALMOLOGICO->{'MIOPIA_OBS'};

    $USO_LENTES = ($key->Data->SERV_MED_OFTALMOLOGICO->{'USO_LENTES'} == 0) ? "No" : "Sí";
    $ASTIGMATISMO = ($key->Data->SERV_MED_OFTALMOLOGICO->{'ASTIGMATISMO'} == 0) ? "No" : "Sí";
    $PRESBICIA = ($key->Data->SERV_MED_OFTALMOLOGICO->{'PRESBICIA'} == 0) ? "No" : "Sí";
    $ESTRABISMO = ($key->Data->SERV_MED_OFTALMOLOGICO->{'ESTRABISMO'} == 0) ? "No" : "Sí";
    $DALTONISMO = ($key->Data->SERV_MED_OFTALMOLOGICO->{'DALTONISMO'} == 0) ? "No" : "Sí";
    $DESPENDIMIENTO_RET = ($key->Data->SERV_MED_OFTALMOLOGICO->{'DESPENDIMIENTO_RET'} == 0) ? "No" : "Sí";
    $MIOPIA = ($key->Data->SERV_MED_OFTALMOLOGICO->{'MIOPIA'} == 0) ? "No" : "Sí";
    ...
    $HERNIAS = ($key->Data->SERV_MED_DIGESTIVO->{'HERNIAS'} == 0) ? "No" : "Sí";
    $CIRROSIS = ($key->Data->SERV_MED_DIGESTIVO->{'CIRROSIS'} == 0) ? "No" : "Sí";
    $COLITIS = ($key->Data->SERV_MED_DIGESTIVO->{'COLITIS'} == 0) ? "No" : "Sí";
    $GASTRITIS = ($key->Data->SERV_MED_DIGESTIVO->{'GASTRITIS'} == 0) ? "No" : "Sí";
    $HEMORROIDES = ($key->Data->SERV_MED_DIGESTIVO->{'HEMORROIDES'} == 0) ? "No" : "Sí";
    $CIRUGIAS = ($key->Data->SERV_MED_DIGESTIVO->{'CIRUGIAS'} == 0) ? "No" : "Sí";
    ...
    $INCONTINENSIA = ($key->Data->SERV_MED_RENAL->{'INCONTINENSIA'} == 0) ? "No" : "Sí";
    $INSUFICIENCIA = ($key->Data->SERV_MED_RENAL->{'INSUFICIENCIA'} == 0) ? "No" : "Sí";
    $LITIASIS = ($key->Data->SERV_MED_RENAL->{'LITIASIS'} == 0) ? "No" : "Sí";
    ...
    $ANEURISMA = ($key->Data->SERV_MED_SIST_NERVIOSO->{'ANEURISMA'} == 0) ? "No" : "Sí";
    $ENF_DEG_MUS_NEU = ($key->Data->SERV_MED_SIST_NERVIOSO->{'ENF_DEG_MUS_NEU'} == 0) ? "No" : "Sí";
    $ACC_CER_VASCU = ($key->Data->SERV_MED_SIST_NERVIOSO->{'ACC_CER_VASCU'} == 0) ? "No" : "Sí";
    $EPILEPSIA2 = ($key->Data->SERV_MED_SIST_NERVIOSO->EPILEPSIA == 0) ? "No" : "Sí";
    $PAR_FACIAL = ($key->Data->SERV_MED_SIST_NERVIOSO->{'PAR_FACIAL'} == 0) ? "No" : "Sí";
    ...
    $ESCOLIOSIS = ($key->Data->SERV_MED_MUS_ESQUELETICO->{'ESCOLIOSIS'} == 0) ? "No" : "Sí";
    $XIFOSIS = ($key->Data->SERV_MED_MUS_ESQUELETICO->{'XIFOSIS'} == 0) ? "No" : "Sí";
    $LORDOSISI = ($key->Data->SERV_MED_MUS_ESQUELETICO->{'LORDOSISI'} == 0) ? "No" : "Sí";
    $LUMBALGIA = ($key->Data->SERV_MED_MUS_ESQUELETICO->{'LUMBALGIA '} == 0) ? "No" : "Sí"; // Fijate que hay un espacio en 'LUMBALGIA '
    $BASCULAMIENTO = ($key->Data->SERV_MED_MUS_ESQUELETICO->{'BASCULAMIENTO'} == 0) ? "No" : "Sí";
    $ALTERACION_DISCAL = ($key->Data->SERV_MED_MUS_ESQUELETICO->{'ALTERACION_DISCAL'} == 0) ? "No" : "Sí";
    ...
    $ANEURISMA2 = ($key->Data->SERV_MED_CARDIO_NO_PATO->{'ANEURISMA'} == 0) ? "No" : "Sí";
    $HIPERTENSION2 = ($key->Data->SERV_MED_CARDIO_NO_PATO->{'HIPERTENSION'} == 0) ? "No" : "Sí";
    $HIPOTENSION2 = ($key->Data->SERV_MED_CARDIO_NO_PATO->{'HIPOTENSION'} == 0) ? "No" : "Sí";
    $INFARTOS = ($key->Data->SERV_MED_CARDIO_NO_PATO->{'INFARTOS '} == 0) ? "No" : "Sí";
    $INSUFIC_CAR = ($key->Data->SERV_MED_CARDIO_NO_PATO->{'INSUFIC_CAR'} == 0) ? "No" : "Sí";

    /* pt7 */
    $ALRGNEWINP = $key->Data->SERV_MED_TOXI_NO_PATO->{'ALERGIAS'};
    $ALRGNEWINPOBS = $key->Data->SERV_MED_TOXI_NO_PATO->{'ALERGIAS_OBS'};
    ...
    $ALRGNEWINP = ($key->Data->SERV_MED_TOXI_NO_PATO->{'ALERGIAS'} == 0) ? "No" : "Sí";
    $TABAQUISMO2 = ($key->Data->SERV_MED_TOXI_NO_PATO->{'TABAQUISMO'} == 0) ? "No" : "Sí";
    $ALCOHOLISMO2 = ($key->Data->SERV_MED_TOXI_NO_PATO->{'ALCOHOLISMO'} == 0) ? "No" : "Sí";
    $DROGAS2 = ($key->Data->SERV_MED_TOXI_NO_PATO->{'DROGAS'} == 0) ? "No" : "Sí";
    $ALT_CARGA = ($key->Data->SERV_MED_TOXI_NO_PATO->{'ALT_CARGA '} == 0) ? "No" : "Sí";
    $PERFORACIONES = ($key->Data->SERV_MED_TOXI_NO_PATO->{'PERFORACIONES'} == 0) ? "No" : "Sí";
    $TATUAJES = ($key->Data->SERV_MED_TOXI_NO_PATO->{'TATUAJES'} == 0) ? "No" : "Sí";
    ...
    $DIABETES = ($key->Data->SERV_MED_ENDOCRI_NO_PATO->DIABETES == 0) ? "No" : "Sí";
    $HIPERTIROIDISMO2 = ($key->Data->SERV_MED_ENDOCRI_NO_PATO->HIPERTIROIDISMO == 0) ? "No" : "Sí";
    $HIPOTIROIDISMO2 = ($key->Data->SERV_MED_ENDOCRI_NO_PATO->HIPOTIROIDISMO == 0) ? "No" : "Sí";
    ...
    $SATISFACCION = ($key->Data->SERV_MED_EXPLORACION_FISICA->{"SATISFACCION"} == 0) ? "No" : "Sí";
    $COMP_TRABAJO = ($key->Data->SERV_MED_EXPLORACION_FISICA->{"COMP_TRABAJO"} == 0) ? "No" : "Sí";
    $COMP_ORGANIZACION = ($key->Data->SERV_MED_EXPLORACION_FISICA->{"COMP_ORGANIZACION"} == 0) ? "No" : "Sí";
    ...
    $LENGUAJE = ($key->Data->SERV_MED_EXPLORACION_FISICA->{"LENGUAJE"} == 0) ? "No" : "Sí";
    $CONVER_FLUIDA = ($key->Data->SERV_MED_EXPLORACION_FISICA->{"CONVER_FLUIDA"} == 0) ? "No" : "Sí";
    $COHERENCIA_PALABLAS = ($key->Data->SERV_MED_EXPLORACION_FISICA->{"COHERENCIA_PALABLAS"} == 0) ? "No" : "Sí";
    ...
    $FIRMA = $key->Data->SERV_MED_RESULTADO_EXAMEN->{"FIRMA_DIGITAL"};
}

$jsonUser =  $model->searchUser($idNss);
...
$tipoExamenInputO = $others->selectDatsExMed($data = [
    "nss" => $idNss,
    "type" => "tipoExamenInputO"
]);
switch ($tipoExamenInputO) {
    case '0':
        $tipoExamenInputO = "No seleccionado";
        break;
    case '1':
        $tipoExamenInputO = "Admisión";
        break;
    case '2':
        $tipoExamenInputO = "Périodico";
        break;
    case '3':
        $tipoExamenInputO = "Cambio de rol";
        break;
    case '4':
        $tipoExamenInputO = "Post incapacidad";
        break;
    case '5':
        $tipoExamenInputO = "Especial";
        break;
}
...
$LENTEOJODERECHOEXP = ($LENTEOJODERECHOEXP == 0) ? "No" : "Sí";
$LENTEOJOIZQUIERDOEXP = ($LENTEOJOIZQUIERDOEXP == 0) ? "No" : "Sí";
$ROMBERGEXFIS = ($ROMBERGEXFIS == 0) ? "Negativo" : "Positivo";

// Función para comprobar y convertir el valor si es necesario
function comprobarValor($valor)
{
    // Convertir a minúsculas para hacer la comprobación más flexible
    $valor = strtolower($valor);
    // Comprobar si el valor es "sin observación", "sin obs" o "vacio"
    if ($valor == "sin observación" || $valor == "sin obs" || $valor == "vacio" || empty($valor) || $valor == "sin observaciones") {
        return "Negado";
    } else {
        return $valor;
    }
}
function comprobarValor2($valor)
{
    // Convertir a minúsculas para hacer la comprobación más flexible
    $valor = strtolower($valor);
    // Comprobar si el valor es "sin observación", "sin obs" o "vacio"
...
function evaluarCondicionDental($DIENTE)
{
    switch ($DIENTE) {
        case 'C':
        ...
        case 'PPD':
            return 'Profundidad de bolsas periodontales';
        case 'G':
            return 'Gingivitis';
        case 'PI':
        ...
        case 'S':
            return 'Sarro';
        case 'F':
            return 'Fractura dental';
        case 'R':
            return 'Restauración dental';
        case 'RC':
            return 'Relación céntrica';
        case 'O':
            return 'Oclusión';
        case 'V':
            return 'Vitalidad pulpar';
        case 'E':
            return 'Endodoncia';
        case 'D':
            return 'Desgaste dental';
        case 'ATM':
            return 'Articulación temporomandibular';
        case 'Sx':
            return 'Síntomas';
        case 'Tx':
            return 'Tratamiento';
...
<style  media="print" >
	...
	font-size:13px;
		font-weight: bold;
         ...
         @top-right {
	  content: "<?php echo date("d/m/y, H:i A"); ?> ";
 	  font-weight: bold;
          font-size:10px;
	  font-family: system-ui, sans-serif;
          text-align: left;
...
<body class="container border">
    <!-- <div class="col-12 draggable" id="miDiv"></div> -->
    <div class="row" id="">
                ...
                <tbody>
                    <tr>
                        <td colspan="2" style="font-size: 12px;font-weight:bold;">
                            <center>CONSENTIMIENTO DE REALIZACIÓN DE EXAMEN MÉDICO<br/> (<?= $tipoExamenInputO ?>).</center>
                        </td>
                    </tr>
                    <tr class="">
                        <td>F. Implementación: 10 DE ENERO DEL 2023</td>
                        <td>Revisión: 03</td>


                    </tr>
                    <td>Clave de control: FT-SO-11</td>
                    ...
                    <tr class="">
                        ...
                        <td>Motivo del cambio: Revisión por vigencia.</td>
                        <td></td>
    ...
    <div class="row mt-3 ">
        <div class="col-4">
            <p  style="font-size: 13px;">Agencia: <b><?= $empresa ?></b></p>
        </div>
        <div class="col-4">
            <p  style="font-size: 13px;">Departamento: <?php echo $cuenta; ?></p>
        </div>
        <div class="col-4">
            <p  style="font-size: 13px;">Puesto: <b><?= $puesto ?></b></p>
        </div>
    </div>
    <div class="container">
        <p style="font-size: 11px; margin-top: 150px;">
            Entiendo que, de acuerdo con la Política de la Compañía sobre proporcionar y mantener un ambiente de trabajo seguro y saludable para sus empleados, me someteré a la realización de un Examen Médico.
        </p>
        <p style="font-size: 11px;">Por medio del presente autorizo la realización del Examen Médico de Admisión, Periódico o Especiales, el cual consiste en 2 etapas, exploración física e Interrogación Verbal que consta de: </p>
        <p style="font-size: 11px;">
            Ficha de Identificación, Antecedentes Laborales, Antecedentes Heredo familiares, Antecedentes Personales no patológicos, Antecedentes Ginecoobstetricos, Antecedentes Personales Patológicos.
        </p>
        <p style="font-size: 11px;">
            Exploración Física que consta de: Peso, Talla, Toma de Signos Vitales, revisión de Cráneo, Nariz, Boca, Oídos, Cuello, Tórax , Columna Vertebral, Abdomen, Urinario, Extremidades y Piel. Tomando en cuenta que para ello, me tendré que retirar la ropa conservando mi ropa interior, así como se me pedirá que realice algunos movimientos.
        </p>
        <p style="font-size: 11px;">
            Esto será realizado por personal altamente calificado y profesional.
        </p>
    </div>
    ...
    <div class="row mt-3">
        <div class="col-6">
            <center>
                ...
                <img src="<?= $FIRMA  ?>" class="img-fluid" width="50px" alt=""><br>
                <p style="font-size:12px;font-weight:bold"><?php echo $apellidoP; ?> <?php echo $apellidoM; ?> <?php echo $nombre; ?></p>
            </center>
        </div>
        <div class="col-6">
            <center>
                <h6>Fecha</h6><br/>
                <p  style="font-size:13px;font-weight:bold"><?= $FECHAExam; ?></p>
            </center>
...
<div class="page"></div>
        ...
        <div class="col-9">
            <table class="table table-bordered table-sm border-top border-dark">
                ...
                <tbody>
                    <tr>
                        <td colspan="2">
                            <center>AVISO DE PRIVACIDAD</center>
                        </td>
                    </tr>
                    <tr class="">
                        <td>F. Implementación: 18 DE OCTUBRE DE 2022</td>
                        <td>Revisión: 01</td>


                    </tr>
                    <td>Clave de control: FT-SO-32</td>
                    <td>Reemplaza a: 00</td>
                    <tr>
    ...
    <div class=" mt-3 mb-3 ">
        ...
        <p style="font-size: 11px;">SALUD OCUPACIONAL DE GRUPO LOGISTICO INTEGRAL, mejor conocido como ONEST LOGISTICS, con domicilio en calle
            Autopista México Querétaro Km 34.5 Nave 5, colonia San Isidro, ciudad Cuautitlán Izcalli, municipio o delegación Cuautitlán Izcalli,
            c.p. 54730, en la entidad de Estado de México, país México, y portal de internet https://www.onestlogistics.com, es el responsable del
            uso y protección de sus datos personales, y al respecto le informamos lo siguiente:</p>
        <p style="font-size: 11px;">
            ¿Para qué fines utilizaremos sus datos personales? Los datos personales que recabamos de usted, los utilizaremos para las
            siguientes finalidades que son necesarias para el servicio que solicita: Conservar información médica de otras instituciones en
            expediente médico laboral. De manera adicional, utilizaremos su información personal para las siguientes finalidades secundarias que
            no son necesarias para el servicio solicitado, pero que nos permiten y facilitan brindarle una mejor atención: Obtener muestras
            biológicas en los casos que sean necesarios para evaluar las aptitudes médicas o para algún tamizaje de diagnóstico Conocer los
            datos de índole médico para evaluar el estado de salud para ingresar, cambiar de puesto, ascender, de manera periódica o concluir
            su relación laboral. En caso de que no desee que sus datos personales se utilicen para estos fines secundarios, indíquelo a
            continuación:
        </p>
        <p style="font-size: 11px;">
            Sí consiento que mis datos personales se utilicen para los siguientes fines:
            <br>
            <b>[*]Obtener muestras biológicas en los casos que sean necesarios para evaluar las aptitudes médicas o para algún tamizaje
                de diagnóstico.</b><br>
            <b>[*]Conocer los datos de índole médico para evaluar el estado de salud para ingresar, cambiar de puesto, ascender, de
                manera periódica o concluir su relación laboral.</b>
        </p>
        <p style="font-size: 11px;">
            ...
            productos que solicita o contrata con nosotros. ¿Qué datos personales utilizaremos para estos fines? Para llevar a cabo las finalidades
            descritas en el presente aviso de privacidad, utilizaremos los siguientes datos personales: Nombre, Estado Civil, Lugar de nacimiento,
            Fecha de nacimiento, Nacionalidad, Domicilio, Teléfono particular, Teléfono celular, Correo electrónico, Firma autógrafa, Edad,
            Fotografía, Señas particulares, Estatura, Peso, Cicatrices, Tipo de sangre, Puesto o cargo que desempeña, Domicilio de trabajo,
            Correo electrónico institucional, Teléfono institucional, Referencias laborales, Información generada durante los procesos de
            reclutamiento, selección y contratación, Capacitación laboral, Trayectoria educativa, Títulos, Bienes muebles, Pasatiempos, Aficiones,
            Deportes que practica, Juegos de su interés. Además de los datos personales mencionados anteriormente, para las finalidades
            informadas en el presente aviso de privacidad utilizaremos los siguientes datos personales considerados como sensibles, que
            requieren de especial protección: Religión que profesa, Pertenencia a un sindicato, Estado de salud físico presente, pasado o futuro,
            Estado de salud mental presente, pasado o futuro, Información genética, Preferencias sexuales, Prácticas o hábitos sexuales,
            Pertenencia a un pueblo, etnia o región ¿Con quién compartimos su información personal y para qué fines?
        </p>
        <p style="font-size: 11px;">
            Le informamos que sus datos personales son compartidos dentro del país con las siguientes personas, empresas, organizaciones o
            autoridades distintas a nosotros, para los siguientes fines: Destinatario de los datos personales: P&G Certificación de Aptitud Médica
            Laboral, Unilever de México, Certificación de Aptitud Médica Laboral, Inditex, Certificación de Aptitud Médica Laboral, u Otros clientes
            Certificación de Aptitud Médica Laboral.
        </p>
        <p style="font-size: 11px;">
            Con relación a las transferencias que requieren de su consentimiento, por favor indique a continuación si nos lo otorga:
        </p>
        <b class="form-check-label" style="font-size: 11px;" for="exampleCheck1">[*]Sí otorgo mi consentimiento para las siguientes transferencias de mis datos personales: P&G, Unilever de México, Inditex
            y Otros clientes.</b>
        <p style="font-size: 11px;">
        ...
        <p style="font-size: 11px;">
            ...
            está siendo utilizada adecuadamente (Cancelación); así como oponerse al uso de sus datos personales para fines específicos
            (Oposición). Estos derechos se conocen como derechos ARCO. Para el ejercicio de cualquiera de los derechos ARCO, usted deberá
            presentar la solicitud respectiva a través del siguiente medio: Número telefónico 5563595902 Para conocer el procedimiento y
            requisitos para el ejercicio de los derechos ARCO, ponemos a su disposición el siguiente medio: Número telefónico 5563595902 Los
            datos de contacto de la persona o departamento de datos personales, que está a cargo de dar trámite a las solicitudes de derechos
        ...
        <p style="font-size: 11px;">
            a) Nombre de la persona o departamento de datos personales: Salud Ocupacional
        </p>
        <p style="font-size: 11px;">
            b) Domicilio: calle Carretera México Querétaro Km 34.5 Nave 5, colonia San Isidro, ciudad Cuautitlán Izcalli, municipio o delegación
            Cuautitlán Izcalli, C.P. 54730, en la entidad de Estado de México, país México
        </p>
        <p style="font-size: 11px;">
            C) Correo electrónico: saludocupacional@grupoonest.com d) Número telefónico: 5563595902.
        </p>
        <p style="font-size: 11px;">
            Usted puede revocar su consentimiento para el uso de sus datos personales, Usted puede revocar el consentimiento que, en su caso,
            nos haya otorgado para el tratamiento de sus datos personales. Sin embargo, es importante que tenga en cuenta que no en todos los
            casos podremos atender su solicitud o concluir el uso de forma inmediata, ya que es posible que por alguna obligación legal requiramos
            seguir tratando sus datos personales. Asimismo, usted deberá considerar que, para ciertos fines, la revocación de su consentimiento
            implicará que no le podamos seguir prestando el servicio que nos solicitó, o la conclusión de su relación con nosotros. Para revocar
            su consentimiento deberá presentar su solicitud a través del siguiente medio: Número telefónico 5563595902 Con relación al
            procedimiento y requisitos para la revocación de su consentimiento, le informamos lo siguiente:
        </p>
        <p style="font-size: 11px;">
            a) ¿A través de qué medios pueden acreditar su identidad el titular y, en su caso, su representante, así como la personalidad este
            último? Identificación oficial

        </p>
        <p style="font-size: 11px;">
            b) ¿Qué información y/o documentación deberá contener la solicitud? Solicitud puntual
        </p>
        <p style="font-size: 11px;">
            c) ¿En cuántos días le daremos respuesta a su solicitud? 72 horas hábiles
        </p>
        <p style="font-size: 11px;">d) ¿Por qué medio le comunicaremos la respuesta a su solicitud? Correo electrónico</p>
        <p style="font-size: 11px;">
            e) Para más información sobre el procedimiento, ponemos a disposición los siguientes medios: Número telefónico 5563595902
        </p>
        <p style="font-size: 11px;">
            ¿Cómo puede limitar el uso o divulgación de su información personal? Con objeto de que usted pueda limitar el uso y divulgación de
            su información personal, le ofrecemos los siguientes medios: Número telefónico 5563595902 ¿Cómo puede conocer los cambios en
            este aviso de privacidad? El presente aviso de privacidad puede sufrir modificaciones, cambios o actualizaciones derivadas de nuevos
            requerimientos legales; de nuestras propias necesidades por los productos o servicios que ofrecemos; de nuestras prácticas de
            privacidad; de cambios en nuestro modelo de negocio, o por otras causas. Nos comprometemos a mantenerlo informado sobre los
            cambios que pueda sufrir el presente aviso de privacidad, a través de: Carteles a través de cada consultorio médico.
        </p>
        <p style="font-size: 11px;">El procedimiento a través del cual se llevarán a cabo las notificaciones sobre cambios o actualizaciones al presente aviso de privacidad
            es el siguiente:
        </p>
        <p style="font-size: 11px;">Por medio de cartel en el consultorio médico Su consentimiento para el tratamiento de sus datos personal</p>
        <b class="form-check-label" style="font-size: 11px;" for="exampleCheck2">[*]Sí consiento que mis datos personales sean tratados de conformidad con los términos y condiciones informados en el
	    presente aviso de privacidad.</b>
<br/><br/>
        <center>
            <h6>Nombre y firma</h6>
            <img src="<?= $FIRMA  ?>" class="img-fluid" width="50px" alt=""><br>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <tr>
                    ...
                    <td><?= $contactoEmernombre1 ?></td>
                    <td><?= $contactoEmerapellido_paterno1 ?></td>
                    <td><?= $contactoEmerapellido_materno1 ?></td>
                    <td><?= $contactoEmerparentesco1 ?></td>
                    <td><?= $contactoEmertelefono1 ?></td>
                </tr>
                <tr>

                    <td><?= $contactoEmernombre2 ?></td>
                    <td><?= $contactoEmerapellido_paterno2 ?></td>
                    <td><?= $contactoEmerapellido_materno2 ?></td>
                    <td><?= $contactoEmerparentesco2 ?></td>
                    <td><?= $contactoEmertelefono2 ?></td>
                </tr>
                <tr>

                    <td><?= $contactoEmernombre3 ?></td>
                    <td><?= $contactoEmerapellido_paterno3 ?></td>
                    <td><?= $contactoEmerapellido_materno3 ?></td>
                    <td><?= $contactoEmerparentesco3 ?></td>
                    <td><?= $contactoEmertelefono3 ?></td>
                </tr>
            </tbody>
        </table>
    </div>
        ...
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td><?php echo $avc = comprobarValor($avc); ?></td>
                    <td><?php echo $AVC_OBS = comprobarValor($AVC_OBS); ?></td>
                </tr>
                <tr>
                    <td>Epilepsia</td>
                    <td><?php echo $EPILEPSIA = comprobarValor($EPILEPSIA); ?></td>
                    <td><?php echo $EPILEPSIA_OBS = comprobarValor($EPILEPSIA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Parálisis Facial</td>
                    <td><?php echo $PARALISIS_FACIAL = comprobarValor($PARALISIS_FACIAL); ?></td>
                    <td><?php echo $PARALISIS_FACIAL_OBS = comprobarValor($PARALISIS_FACIAL_OBS); ?></td>
                </tr>
                <tr>
                    <td>Parkinson</td>
                    <td><?php echo $PARKINSON = comprobarValor($PARKINSON); ?></td>
                    <td><?php echo $PARKINSON_OBS = comprobarValor($PARKINSON_OBS); ?></td>
                </tr>
                <tr>
                    <td>Alzheimer</td>
                    <td><?php echo $ALZHEIMER = comprobarValor($ALZHEIMER); ?></td>
                    <td><?php echo $ALZHEIMER_OBS = comprobarValor($ALZHEIMER_OBS); ?></td>
                </tr>
                <tr>
                    <td>Migraña</td>
                    <td><?php echo $MIGRAÑA = comprobarValor($MIGRAÑA); ?></td>
                    <td><?php echo $MIGRAÑA_OBS = comprobarValor($MIGRAÑA_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td><?php echo $CARDIOPATIA_ISQUEMICA = comprobarValor($CARDIOPATIA_ISQUEMICA); ?></td>
                    <td><?php echo $CARDIOPATIA_ISQUEMICA_OBS = comprobarValor($CARDIOPATIA_ISQUEMICA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Hipertensión</td>
                    <td><?php echo $HIPERTENSION = comprobarValor($HIPERTENSION); ?></td>
                    <td><?php echo $HIPERTENSION_OBS = comprobarValor($HIPERTENSION_OBS); ?></td>
                </tr>
                <tr>
                    <td>Hipotensión</td>
                    <td><?php echo $HIPOTENSION = comprobarValor($HIPOTENSION); ?></td>
                    <td><?php echo $HIPOTENSION_OBS = comprobarValor($HIPOTENSION_OBS); ?></td>
                </tr>
                <tr>
                    <td>Insuficiencia Cardíaca</td>
                    <td><?php echo $INSUFICIENCIA_CARDIACA = comprobarValor($INSUFICIENCIA_CARDIACA); ?></td>
                    <td><?php echo $INSUFICIENCIA_CARDIACA_OBS = comprobarValor($INSUFICIENCIA_CARDIACA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Insuficiencia Vascular Periférica</td>
                    <td><?php echo $INSUFICIENCIA_VASCULAR_PERIFERICA = comprobarValor($INSUFICIENCIA_VASCULAR_PERIFERICA); ?></td>
                    <td><?php echo $INSUFICIENCIA_VASCULAR_PERIFERICA_OBS = comprobarValor($INSUFICIENCIA_VASCULAR_PERIFERICA_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Bronquitis</td>
                    <td><?php echo $BRONQUITIS = comprobarValor($BRONQUITIS); ?></td>
                    <td><?php echo $BRONQUITIS_OBS = comprobarValor($BRONQUITIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Tuberculosis</td>
                    <td><?php echo $TUBERCULOSIS = comprobarValor($TUBERCULOSIS); ?></td>
                    <td><?php echo $TUBERCULOSIS_OBS = comprobarValor($TUBERCULOSIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Asma</td>
                    <td><?php echo $ASMA = comprobarValor($ASMA); ?></td>
                    <td><?php echo $ASMA_OBS = comprobarValor($ASMA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Insuficiencia Respiratoria</td>
                    <td><?php echo $INSUFICIENCIA_RESPIRATORIA = comprobarValor($INSUFICIENCIA_RESPIRATORIA); ?></td>
                    <td><?php echo $INSUFICIENCIA_RESPIRATORIA_OBS = comprobarValor($INSUFICIENCIA_RESPIRATORIA_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Alcoholismo</td>
                    <td><?php echo $ALCOHOLISMO = comprobarValor($ALCOHOLISMO); ?></td>
                    <td><?php echo $ALCOHOLISMO_OBS = comprobarValor($ALCOHOLISMO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Tabaquismo</td>
                    <td><?php echo $TABAQUISMO = comprobarValor($TABAQUISMO); ?></td>
                    <td><?php echo $TABAQUISMO_OBS = comprobarValor($TABAQUISMO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Drogas</td>
                    <td><?php echo $DROGAS = comprobarValor($DROGAS); ?></td>
                    <td><?php echo $DROGAS_OBS = comprobarValor($DROGAS_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td><?php echo $IRC = comprobarValor($IRC); ?></td>
                    <td><?php echo $IRC_OBS = comprobarValor($IRC_OBS); ?></td>
                </tr>
                <tr>
                    <td>Litiasis Renal</td>
                    <td><?php echo $LITASIS = comprobarValor($LITASIS); ?></td>
                    <td><?php echo $LITASIS_OBS = comprobarValor($LITASIS_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td><?php echo $DM = comprobarValor($DM); ?></td>
                    <td><?php echo $DM_OBS = comprobarValor($DM_OBS); ?></td>
                </tr>
                <tr>
                    <td>Hipertiroidismo</td>
                    <td><?php echo $HIPERTIROIDISMO = comprobarValor($HIPERTIROIDISMO); ?></td>
                    <td><?php echo $HIPERTIROIDISMO_OBS = comprobarValor($HIPERTIROIDISMO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Hipotiroidismo</td>
                    <td><?php echo $HIPOTIROIDISMO = comprobarValor($HIPOTIROIDISMO); ?></td>
                    <td><?php echo $HIPOTIROIDISMO_OBS = comprobarValor($HIPOTIROIDISMO_OBS); ?></td>
                </tr>
            </table>
        </div>
        ...
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td><?php echo $TRANSTORNO_DE_CRECIMIENTO = comprobarValor($TRANSTORNO_DE_CRECIMIENTO); ?></td>
                    <td><?php echo $TRANSTORNO_DE_CRECIMIENTO_OBS = comprobarValor($TRANSTORNO_DE_CRECIMIENTO_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Neurosis</td>
                    <td><?php echo $NEUROSIS = comprobarValor($NEUROSIS); ?></td>
                    <td><?php echo $NEUROSIS_OBS = comprobarValor($NEUROSIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Psicosis</td>
                    <td><?php echo $PSICOSIS = comprobarValor($PSICOSIS); ?></td>
                    <td><?php echo $PSICOSIS_OBS = comprobarValor($PSICOSIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Intento de Suicidio</td>
                    <td><?php echo $INTENTO_DE_SUICIDIO = comprobarValor($INTENTO_DE_SUICIDIO); ?></td>
                    <td><?php echo $INTENTO_DE_SUICIDIO_OBS = comprobarValor($INTENTO_DE_SUICIDIO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Esquizofrenia</td>
                    <td><?php echo $ESQUIZOFRENIA = comprobarValor($ESQUIZOFRENIA); ?></td>
                    <td><?php echo $ESQUIZOFRENIA_OBS = comprobarValor($ESQUIZOFRENIA_OBS); ?></td>
                </tr>
            </table>
	</div>
<div class="page"></div>
        ...
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Cáncer</td>
                    <td><?php echo $CANCER = comprobarValor($CANCER); ?></td>
                    <td><?php echo $CANCER_OBS = comprobarValor($CANCER_OBS); ?></td>
                </tr>
                <tr>
                    <td>Alergias</td>
                    <td><?php echo $ALERGIAS = comprobarValor($ALERGIAS); ?></td>
                    <td><?php echo $ALERGIAS_OBS = comprobarValor($ALERGIAS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Malformaciones</td>
                    <td><?php echo $MALFORMACIONES = comprobarValor($MALFORMACIONES); ?></td>
                    <td><?php echo $MALFORMACIONES_OBS = comprobarValor($MALFORMACIONES_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Otras</td>
                    <td><?php echo $OTRAS = comprobarValor($OTRAS); ?></td>
                    <td><?php echo $OTRAS_OBS = comprobarValor($OTRAS_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-12">
            ...
            <div class="row">
                ...
                <div class="col-6">
                    <table class="table table table-bordered table-sm">
                        ...
                        <tr>
                            <td>Baño</td>
                            <td><?php echo $BAÑO; ?> veces por semana</td>
                        </tr>
                        <tr>
                            <td>Cambio de Ropa</td>
                            <td><?php echo $CAMBIO_DE_ROPA; ?> veces por dia</td>
                        </tr>
                        <tr>
                            <td>Aseo Bucal</td>
                            <td><?php echo $ASEO_BUCAL; ?> veces por dia</td>
                        </tr>
                        <tr>
                            <td>Alimentación</td>
                        ...
                        <tr>
                            <td>Frecuencia</td>
                            <td><?php echo $FC1; ?> veces por <?= $cadaCuandoDep ?></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td>Frecuencia</td>
                            <td><?php echo $FC2; ?> veces por <?= $cadaCuandoPasa ?></td>
                        </tr>
                    </table>
                </div>
            </div>
        ...
        <div class="col-12">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Fecha Influenza</td>
                    <td><?php echo $fechaINFLUEN; ?></td>
                </tr>
                <tr>
                    <td>TD</td>
                    <td><?php echo $TD; ?></td>
...
<div class="page"></div>
        ...
        <div class="col-12">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>G (Gestas)</td>
                    <td><?php echo comprobarValor($G); ?></td>
                </tr>
                <tr>
                    <td>A (Abortos)</td>
                    <td><?php echo comprobarValor($A); ?></td>
                </tr>
                <tr>
                    <td>P (Partos)</td>
                    <td><?php echo comprobarValor($P); ?></td>
                </tr>
                <tr>
                    <td>C (Cesáreas)</td>
                    <td><?php echo comprobarValor($C); ?></td>
                </tr>
                <tr>
                    <td>Método de Planificación</td>
                    <td><?php echo comprobarValor($MET_PLANIFICACION); ?></td>
                </tr>
                <tr>
                    <td>Papanicolaou</td>
                    <td><?php echo comprobarValor($PAPANICOLAOU); ?></td>
                </tr>
                <tr>
                    <td>Infecciones Repetitivas</td>
                    <td><?php echo comprobarValor($INFECC_REP); ?></td>
                </tr>
                <tr>
                    <td>Fecha de Última Consulta Ginecológica</td>
                    <td><?php echo comprobarValor($FUP); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-12">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td><?php echo comprobarValor($ENF_CONGENITAS); ?></td>
                </tr>
                <tr>
                    <td>Genital</td>
                    <td><?php echo comprobarValor($GENITAL); ?></td>
                </tr>
                <tr>
                    <td>Enfermedades Propias de la Infancia</td>
                    <td><?php echo comprobarValor($ENF_PROP_INF); ?></td>
                </tr>
                <tr>
                    <td>Dermatopatías</td>
                    <td><?php echo comprobarValor($DERMATOPATIAS); ?></td>
                </tr>
                <tr>
                    <td>Sistema Auditivo</td>
                    <td><?php echo comprobarValor($SIST_AUDITIVO); ?></td>
                </tr>
                <tr>
                    <td>Sistema Respiratorio</td>
                    <td><?php echo comprobarValor($RESPIRATORIO); ?></td>
                </tr>
                <tr>
                    <td>Sistema Olfativo</td>
                    <td><?php echo comprobarValor($SIS_OLFATIVO); ?></td>
                </tr>
                <tr>
                    <td>Traumatismos</td>
                    <td><?php echo comprobarValor($TRAUMATICOS); ?></td>
                </tr>
            </table>
        </div>
        ...
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Uso de Lentes</td>
                    <td><?php echo comprobarValor($USO_LENTES); ?></td>
                    <td><?php echo comprobarValor($USO_LENTES_OBS); ?></td>
                </tr>
                <tr>
                    <td>Astigmatismo</td>
                    <td><?php echo comprobarValor($ASTIGMATISMO); ?></td>
                    <td><?php echo comprobarValor($ASTIGMATISMO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Presbicia</td>
                    <td><?php echo comprobarValor($PRESBICIA); ?></td>
                    <td><?php echo comprobarValor($PRESBICIA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Estrabismo</td>
                    <td><?php echo comprobarValor($ESTRABISMO); ?></td>
                    <td><?php echo comprobarValor($ESTRABISMO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Daltonismo</td>
                    <td><?php echo comprobarValor($DALTONISMO); ?></td>
                    <td><?php echo comprobarValor($DALTONISMO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Desprendimiento de Retina</td>
                    <td><?php echo comprobarValor($DESPENDIMIENTO_RET); ?></td>
                    <td><?php echo comprobarValor($DESPENDIMIENTO_RET_OBS); ?></td>
                </tr>
                <tr>
                    <td>Miopia</td>
                    <td><?php echo comprobarValor($MIOPIA); ?></td>
                    <td><?php echo comprobarValor($MIOPIA_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Hernias</td>
                    <td><?php echo comprobarValor($HERNIAS); ?></td>
                    <td><?php echo comprobarValor($HERNIAS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Cirrosis</td>
                    <td><?php echo comprobarValor($CIRROSIS); ?></td>
                    <td><?php echo comprobarValor($CIRROSIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Colitis</td>
                    <td><?php echo comprobarValor($COLITIS); ?></td>
                    <td><?php echo comprobarValor($COLITIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Gastritis</td>
                    <td><?php echo comprobarValor($GASTRITIS); ?></td>
                    <td><?php echo comprobarValor($GASTRITIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Hemorroides</td>
                    <td><?php echo comprobarValor($HEMORROIDES); ?></td>
                    <td><?php echo comprobarValor($HEMORROIDES_OBS); ?></td>
                </tr>
                <tr>
                    <td>Cirugías</td>
                    <td><?php echo comprobarValor($CIRUGIAS); ?></td>
                    <td><?php echo comprobarValor($CIRUGIAS_OBS); ?></td>
                </tr>
            </table>
...
<div class="page"></div>
        ...
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Incontinencia</td>
                    <td><?php echo comprobarValor($INCONTINENSIA); ?></td>
                    <td><?php echo comprobarValor($INCONTINENSIA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Insuficiencia Renal</td>
                    <td><?php echo comprobarValor($INSUFICIENCIA); ?></td>
                    <td><?php echo comprobarValor($INSUFICIENCIA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Litiasis Renal</td>
                    <td><?php echo comprobarValor($LITIASIS); ?></td>
                    <td><?php echo comprobarValor($LITIASIS_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Aneurisma</td>
                    <td><?php echo comprobarValor($ANEURISMA); ?></td>
                    <td><?php echo comprobarValor($ANEURISMA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Enfermedad Degenerativa Muscular/Nerviosa</td>
                    <td><?php echo comprobarValor($ENF_DEG_MUS_NEU); ?></td>
                    <td><?php echo comprobarValor($ENF_DEG_MUS_NEU_OBS); ?></td>
                </tr>
                <tr>
                    <td>Accidente Cerebrovascular</td>
                    <td><?php echo comprobarValor($ACC_CER_VASCU); ?></td>
                    <td><?php echo comprobarValor($ACC_CER_VASCU_OBS); ?></td>
                </tr>
                <tr>
                    <td>Epilepsia</td>
                    <td><?php echo comprobarValor($EPILEPSIA2); ?></td>
                    <td><?php echo comprobarValor($EPILEPSIA_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Parálisis Facial</td>
                    <td><?php echo comprobarValor($PAR_FACIAL); ?></td>
                    <td><?php echo comprobarValor($PAR_FACIAL_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Escoliosis</td>
                    <td><?php echo comprobarValor($ESCOLIOSIS); ?></td>
                    <td><?php echo comprobarValor($ESCOLIOSIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Xifosis</td>
                    <td><?php echo comprobarValor($XIFOSIS); ?></td>
                    <td><?php echo comprobarValor($XIFOSIS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Lordosis</td>
                    <td><?php echo comprobarValor($LORDOSISI); ?></td>
                    <td><?php echo comprobarValor($LORDOSISI_OBS); ?></td>
                </tr>
                <tr>
                    <td>Lumbalgia</td>
                    <td><?php echo comprobarValor($LUMBALGIA); ?></td>
                    <td><?php echo comprobarValor($LUMBALGIA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Basculamiento</td>
                    <td><?php echo comprobarValor($BASCULAMIENTO); ?></td>
                    <td><?php echo comprobarValor($BASCULAMIENTO_OBS); ?></td>
                </tr>
                <tr>
                    <td>Alteración Discal</td>
                    <td><?php echo comprobarValor($ALTERACION_DISCAL); ?></td>
                    <td><?php echo comprobarValor($ALTERACION_DISCAL_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Aneurisma</td>
                    <td><?php echo comprobarValor($ANEURISMA2); ?></td>
                    <td><?php echo comprobarValor($ANEURISMA_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Hipertensión</td>
                    <td><?php echo comprobarValor($HIPERTENSION2); ?></td>
                    <td><?php echo comprobarValor($HIPERTENSION_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Hipotensión</td>
                    <td><?php echo comprobarValor($HIPOTENSION2); ?></td>
                    <td><?php echo comprobarValor($HIPOTENSION_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Infartos</td>
                    <td><?php echo comprobarValor($INFARTOS); ?></td>
                    <td><?php echo comprobarValor($INFARTOS_OBS); ?></td>
                </tr>
                <tr>
                    <td>Insuficiencia Cardíaca</td>
                    <td><?php echo comprobarValor($INSUFIC_CAR); ?></td>
                    <td><?php echo comprobarValor($INSUFIC_CAR_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Alergias</td>
                    <td><?php echo comprobarValor($ALRGNEWINP); ?></td>
                    <td><?php echo comprobarValor($ALRGNEWINPOBS); ?></td>
                </tr>
                <tr>
                    <td>Tabaquismo</td>
                    <td><?php echo comprobarValor($TABAQUISMO2); ?></td>
                    <td><?php echo comprobarValor($TABAQUISMO_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Alcoholismo</td>
                    <td><?php echo comprobarValor($ALCOHOLISMO2); ?></td>
                    <td><?php echo comprobarValor($ALCOHOLISMO_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Drogas</td>
                    <td><?php echo comprobarValor($DROGAS2); ?></td>
                    <td><?php echo comprobarValor($DROGAS_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Alteración de Carga</td>
                    <td><?php echo comprobarValor($ALT_CARGA); ?></td>
                    <td><?php echo comprobarValor($ALT_CARGA_OBS); ?></td>
                </tr>
                <tr>
                    <td>Perforaciones</td>
                    <td><?php echo comprobarValor($PERFORACIONES); ?></td>
                    <td><?php echo comprobarValor($PERFORACIONES_OBS); ?></td>
                </tr>
                <tr>
                    <td>Tatuajes</td>
                    <td><?php echo comprobarValor($TATUAJES); ?></td>
                    <td><?php echo comprobarValor($TATUAJES_OBS); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Diabetes</td>
                    <td><?php echo comprobarValor($DIABETES); ?></td>
                    <td><?php echo comprobarValor($DIABETES_OBS); ?></td>
                </tr>
                <tr>
                    <td>Hipertiroidismo</td>
                    <td><?php echo comprobarValor($HIPERTIROIDISMO2); ?></td>
                    <td><?php echo comprobarValor($HIPERTIROIDISMO_OBS2); ?></td>
                </tr>
                <tr>
                    <td>Hipotiroidismo</td>
                    <td><?php echo comprobarValor($HIPOTIROIDISMO2); ?></td>
                    <td><?php echo comprobarValor($HIPOTIROIDISMO_OBS2); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-12">
...
<div class="page"></div>
        ...
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td><?php echo $FC; ?> x minuto</td>
                </tr>
                <tr>
                    <td>Frecuencia Respiratoria (FR)</td>
                    <td><?php echo $FR; ?> x minuto</td>
                </tr>
                <tr>
                    <td>Tensión Arterial (TA)</td>
                    <td><?php echo $TA; ?> mmHg</td>
                </tr>
                <tr>
                    <td>Temperatura</td>
        ...
        <div class="col-12">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Lente (Ojo Derecho)</td>
                    <td><?php echo $LENTEOJODERECHOEXP; ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Lente (Ojo Izquierdo)</td>
                    <td><?php echo $LENTEOJOIZQUIERDOEXP; ?></td>
                </tr>
                <tr>
                    <td>ROMBERG</td>
                    <td><?php echo $ROMBERGEXFIS; ?></td>
                </tr>
                <tr>
                    <td>Colores</td>
                ...
                <tr>
                    <td>Visión cercana con lentes</td>
                    <td><?php echo $ODEXFISINPCERC; ?></td>
                </tr>
                <tr>
                    <td>Visión cercana sin lentes</td>
                    <td><?php echo $ODEXFISINPCERCIZQ; ?></td>
                </tr>
	    </table> 
<div class="page"></div>
            <div class="col-12" style="">
...
<div class="page"></div>
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Masculino</td>
                    <td><?php echo comprobarValor2($MASCULINO); ?></td>
                </tr>
                <tr>
                    <td>Femenino</td>
                    <td><?php echo comprobarValor2($FEMENINO); ?></td>
                </tr>
            </table>
...
<div class="page"></div>
        ...
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Diente 11</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_11); ?></td>
                </tr>
                <tr>
                    <td>Diente 12</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_12); ?></td>
                </tr>
                <tr>
                    <td>Diente 13</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_13); ?></td>
                </tr>
                <tr>
                    <td>Diente 14</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_14); ?></td>
                </tr>
                <tr>
                    <td>Diente 15</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_15); ?></td>
                </tr>
                <tr>
                    <td>Diente 16</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_16); ?></td>
                </tr>
                <tr>
                    <td>Diente 17</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_17); ?></td>
                </tr>
                <tr>
                    <td>Diente 18</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_18); ?></td>
                </tr>
                <tr>
                    <td>Diente 41</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_41); ?></td>
                </tr>
                <tr>
                    <td>Diente 42</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_42); ?></td>
                </tr>
                <tr>
                    <td>Diente 43</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_43); ?></td>
                </tr>
                <tr>
                    <td>Diente 44</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_44); ?></td>
                </tr>
                <tr>
                    <td>Diente 45</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_45); ?></td>
                </tr>
                <tr>
                    <td>Diente 46</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_46); ?></td>
                </tr>
                <tr>
                    <td>Diente 47</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_47); ?></td>
                </tr>
                <tr>
                    <td>Diente 48</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_48); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-6">
            ...
            <table class="table table-bordered table-sm">
                ...
                <tr>
                    <td>Diente 21</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_2); ?></td>
                </tr>
                <tr>
                    <td>Diente 22</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_22); ?></td>
                </tr>
                <tr>
                    <td>Diente 23</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_23); ?></td>
                </tr>
                <tr>
                    <td>Diente 24</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_24); ?></td>
                </tr>
                <tr>
                    <td>Diente 25</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_25); ?></td>
                </tr>
                <tr>
                    <td>Diente 26</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_26); ?></td>
                </tr>
                <tr>
                    <td>Diente 27</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_27); ?></td>
                </tr>
                <tr>
                    <td>Diente 28</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_28); ?></td>
                </tr>
                <tr>
                    <td>Diente 31</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_31); ?></td>
                </tr>
                <tr>
                    <td>Diente 32</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_32); ?></td>
                </tr>
                <tr>
                    <td>Diente 33</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_33); ?></td>
                </tr>
                <tr>
                    <td>Diente 34</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_34); ?></td>
                </tr>
                <tr>
                    <td>Diente 35</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_35); ?></td>
                </tr>
                <tr>
                    <td>Diente 36</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_36); ?></td>
                </tr>
                <tr>
                    <td>Diente 37</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_37); ?></td>
                </tr>
                <tr>
                    <td>Diente 38</td>
                    <td><?php echo evaluarCondicionDental($DIENTE_38); ?></td>
                </tr>
            </table>
        </div>
        <div class="col-12">
...
<script>
    ...
    // Obtener el elemento que se va a arrastrar
    var elemento = document.getElementById('miDiv');

    // Variables para almacenar la posición inicial del ratón y del elemento
    var offsetX, offsetY;

    // Función que se ejecuta cuando se presiona el ratón sobre el elemento
    function empezarArrastre(event) {
        ...
        // Agregar eventos para mover el elemento cuando el ratón se mueva y dejar de arrastrarlo cuando se suelte el clic
    ...
    // Función que se ejecuta cuando se suelta el clic
    function soltarElemento() {
        // Eliminar los eventos de movimiento y soltar el clic
        document.removeEventListener('mousemove', moverElemento);
        document.removeEventListener('mouseup', soltarElemento);
    }

    // Agregar evento de clic para empezar a arrastrar el elemento
    elemento.addEventListener('mousedown', empezarArrastre);
</script>
