# Plan ETL — Migración de `files` (MariaDB base64) → filesystem + `SERV_MED_FS_FILE`
...
- **Metadatos → tabla `SERV_MED_FS_FILE`** en Oracle (NO el binario).

Se descarta pasar por `MED_FILE` (blob en Oracle): meter 13.5GB a la BD para sacarlos después es trabajo tirado. Se va **directo** MariaDB → filesystem + `SERV_MED_FS_FILE`.
...
1. **`type` es polimórfico:** o categoría funcional (`examen_medico`, etc.) o el id de `consulta_relacionada` (hash MD5). Se copia **`type → FILE_TYPE` tal cual**, sin normalizar. `SERV_MED_FS_FILE.FILE_TYPE` ya está diseñado para esto.
...
`00_init_oracle21c.sql` crea `SERV_MED_UX_FS_FILE_CHECKSUM` **UNIQUE** sobre `SERV_MED_FS_FILE.CHECKSUM_SHA256`. Con 261 contenidos duplicados, el `INSERT` de la segunda copia choca con **`ORA-00001`**.
...
- `SERV_MED_FS_FILE` está **vacía en local y QA** → el cambio es gratis ahora.
- Alternativa descartada: dedup real (un binario, N metadatos) exige otro esquema por solo 1.2%.

**Acción pendiente:** `DROP INDEX SERV_MED_UX_FS_FILE_CHECKSUM` + recrear como índice normal, corregir `00_init`, aplicar en local + QA (igual que el reconcile). **← decidir antes de escribir el runner.**

**✅ Aplicado 2026-08-13 en Oracle local (`PROYECTO_BASE_PDB`).** Verificado: `SERV_MED_UX_FS_FILE_CHECKSUM` ahora `NONUNIQUE`/`VALID`. `00_init_oracle21c.sql` corregido para reflejar esto en instalaciones futuras. **Pendiente: aplicar el mismo `DROP`+`CREATE` en QA** cuando se decida correr la migración ahí (la corrida completa de este ETL será contra Oracle local por ahora, según decisión del 2026-08-13).
...
- Los links "Ver/Descargar" (`FileStoreController.java`) **no son URLs fijas** — se regeneran en cada render a partir de lo que exista en `SERV_MED_FS_FILE` en ese momento. Hoy (sin migrar) las pantallas de archivos históricos simplemente se ven vacías ("sin archivos adjuntos"), no rotas. Después de migrar, aparecen en las mismas pantallas sin distinguir viejo/nuevo.
...
- `FsFileRepository` (`catalog/file/repository/`): `INSERT` a `SERV_MED_FS_FILE` vía JdbcTemplate.
- `SERV_MED_FS_FILE`: tabla de metadatos, ya reconciliada en local + QA (19 columnas).
...
3. **Idempotencia:** `BUSINESS_KEY = "legacy-<id de MariaDB>"`. Aprovecha el índice único `SERV_MED_UX_FS_FILE_BKEY` → re-ejecutar no duplica y permite reanudar si truena a media corrida.
...
- **Target aislado:** XE local + carpeta temporal (`portal.files.root=C:/portal-salud/files-sample`), NO producción ni QA. Si sale mal: borrar carpeta + `TRUNCATE SERV_MED_FS_FILE` y reintentar.
- **Validación manual (el punto de la muestra):**
  - Un par de PDF que abran en visor; una imagen que se vea.
  - `sha256` de `SERV_MED_FS_FILE` coincide al releer el archivo del disco.
...
6. [ ] **Verificar totales:** `COUNT(*)` en `SERV_MED_FS_FILE` vs `files`; sumar `SIZE_BYTES` vs espacio en disco; muestreo aleatorio de apertura de archivos.
...
Sin binarios. `content` es texto (JSON/HTML). Copia directa de filas a `SERV_MED_TAG`, `longtext → CLOB`. Único cuidado: **encoding** — si la BD legacy es `latin1` y no `utf8mb4`, los acentos de los campos médicos se corrompen. Verificar con `SHOW CREATE TABLE tags` antes.
