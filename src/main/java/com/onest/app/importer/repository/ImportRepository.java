...
/**
 * APP_IMPORT_LOTE / APP_IMPORT_FILA (db/sql/app_domain/app-import.sql). JdbcTemplate en vez
...
public class ImportRepository {
    ...
    public boolean disponible() {
        try {
            jdbc.queryForObject("SELECT COUNT(*) FROM APP_IMPORT_LOTE WHERE ROWNUM = 0", Integer.class);
            jdbc.queryForObject("SELECT COUNT(*) FROM APP_IMPORT_FILA WHERE ROWNUM = 0", Integer.class);
            return true;
    ...
    public long insertLote(Long fsFileId, String nombre, String extension, long size, String checksum, int version,
        ...
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO APP_IMPORT_LOTE (FS_FILE_ID, NOMBRE_ARCHIVO, EXTENSION, SIZE_BYTES, CHECKSUM_SHA256, VERSION, "
    ...
    public void insertFilas(long loteId, List<Object[]> filas) {
        jdbc.batchUpdate("INSERT INTO APP_IMPORT_FILA (LOTE_ID, HOJA, FILA_NUM, ESTADO, MENSAJES, VALORES_JSON) VALUES (?,?,?,?,?,?)",
                filas, 500, (ps, f) -> {
                    ps.setLong(1, loteId);
    ...
    public Optional<Lote> findLote(long id) {
        return jdbc.query("SELECT " + COLS + " FROM APP_IMPORT_LOTE WHERE ID = ?", LOTE, id).stream().findFirst();
    }

    public Optional<String> findResumen(long id) {
        return jdbc.query("SELECT RESUMEN_JSON FROM APP_IMPORT_LOTE WHERE ID = ?", (rs, i) -> rs.getString(1), id)
                .stream().findFirst();
    }

    public List<Lote> listar(int limite) {
        return jdbc.query("SELECT " + COLS + " FROM (SELECT " + COLS + " FROM APP_IMPORT_LOTE ORDER BY ID DESC) WHERE ROWNUM <= ?",
                LOTE, limite);
    }

    /** Ultima version viva (no DESCARTADA) de un archivo por nombre. */
    public Optional<Lote> ultimaVersion(String nombreArchivo) {
        return jdbc.query("SELECT " + COLS + " FROM (SELECT " + COLS + " FROM APP_IMPORT_LOTE WHERE UPPER(NOMBRE_ARCHIVO) = UPPER(?) "
    ...
    public int cambiarEstado(long id, String estado) {
        return jdbc.update("UPDATE APP_IMPORT_LOTE SET ESTADO = ? WHERE ID = ?", estado, id);
    }

    public int confirmar(long id, String usuario) {
        return jdbc.update("UPDATE APP_IMPORT_LOTE SET ESTADO = 'CONFIRMADO', CONFIRMED_AT = ?, CONFIRMED_BY = ? "
    ...
    public int descartar(long id) {
        jdbc.update("DELETE FROM APP_IMPORT_FILA WHERE LOTE_ID = ?", id);
        return jdbc.update("UPDATE APP_IMPORT_LOTE SET ESTADO = 'DESCARTADO' WHERE ID = ? AND ESTADO IN ('VALIDADO','SUSTITUIDO')", id);
    }
    ...
    public List<FilaGuardada> filas(long loteId, String hoja, int desde, int cuantas) {
        return jdbc.query("SELECT HOJA, FILA_NUM, ESTADO, MENSAJES, VALORES_JSON FROM ("
                        + "SELECT f.*, ROW_NUMBER() OVER (ORDER BY FILA_NUM) RN FROM APP_IMPORT_FILA f WHERE LOTE_ID = ? AND HOJA = ?) "
                        + "WHERE RN > ? AND RN <= ?",
