...
--
-- Generado el 23-sep-2026 concatenando, EN ORDEN, los scripts de src/main/resources/db/sql.
-- No se modifico ninguna sentencia: es el mismo DDL ya aplicado en QA y en local.
--
...
-- 2. Correr el bloque "PRE-CHECK" de aqui abajo: confirma que ninguno de los objetos que se
--    van a crear existe ya con otra forma. En BIOMETRICO hay 10 tablas APP_* de OTRO sistema
--    (APP_CONFIG_EXCEL, APP_CONSTANTES, APP_DEBUG, APP_EMPLEADO_ACCESOS, APP_ESTADO,
--    APP_REPORTES, APP_TAREAS, APP_TAREA_JSP, APP_UNUSED_CONSTRAINTS, APP_CONFIG_TABLE_EXCEL):
--    verificado el 23-sep, NINGUNA choca por nombre con las del portal, pero hay que
--    comprobarlo de nuevo en el momento de aplicar.
...
-- -------------------------------------------------------------------------------------
-- Objetos del portal que YA existen en este esquema (lo normal la primera vez: 0 filas)
SELECT table_name, 'ya existe' estado FROM user_tables
 WHERE table_name IN ('APP_SEC_USER','APP_SEC_ROLE','APP_SEC_USER_ROLE','APP_SEC_PERMISSION',
                      'APP_SEC_ROLE_PERMISSION','APP_MENU','APP_MENU_ROLE','APP_AUD_EVENT',
                      'APP_AUDIT_SQL_EXECUTION','APP_FS_FILE','APP_FS_FILE_VERSION',
                      'APP_FS_FILE_ACCESS_LOG','APP_FS_FILE_POLICY','APP_FS_FILE_ORPHAN_TRACKING',
                      'APP_IMPORT_LOTE','APP_IMPORT_FILA','APP_JOB_CATALOG','APP_NOTIF_TEMPLATE',
                      'MED_TAG','MED_TAG_MIG_LOG')
 ORDER BY table_name;

-- Las tablas APP_* de OTRO sistema que ya viven aqui (no se tocan)
SELECT table_name FROM user_tables WHERE table_name LIKE 'APP\_%' ESCAPE '\' ORDER BY 1;

-- Privilegios del usuario con el que se va a ejecutar
...
-- Nucleo: APP_SEC_* (seguridad), APP_MENU, APP_AUD_EVENT, APP_FS_FILE + satelites,
--    APP_JOB_CATALOG, APP_NOTIF_TEMPLATE, APP_AUDIT_SQL_EXECUTION, indices, triggers y vistas.
--    Es el script AUTORITATIVO de APP_FS_FILE (crea tambien sus FKs).
-- =====================================================================================
...
BEGIN EXECUTE IMMEDIATE 'CREATE INDEX UX_FS_FILE_CHECKSUM ON APP_FS_FILE(CHECKSUM_SHA256)'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
-- Patrones de acceso de los adjuntos (FsFileRepository): por tipo y por NSS+tipo
BEGIN EXECUTE IMMEDIATE 'CREATE INDEX IDX_APP_FS_FILE_TYPE ON APP_FS_FILE(FILE_TYPE)'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'CREATE INDEX IDX_APP_FS_FILE_NSS_TYPE ON APP_FS_FILE(NSS, FILE_TYPE)'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;
/
...
-- RBAC local: APP_MENU_ROLE + siembra de roles (USER, ADM, ENFERMERO, ROLE_ADMIN,
--    ROLE_MEDICO_ANALISTA) y de los 14 menus del expediente.
-- =====================================================================================
-- 01_rbac_local.sql
...
-- Da de alta en el catalogo de menus (APP_MENU) las 3 pantallas del grupo "Examenes" del
-- sidebar (Antidoping - Seleccion, Consumibles, Causas de consulta) para que se asignen
-- por rol en Administracion > Roles > Menus (APP_MENU_ROLE) igual que los modulos del
...
-- =====================================================================================
MERGE INTO APP_MENU t USING (SELECT 'ANTIDOPING_SELECCION' code, 'Antidoping - Seleccion' title, 'ri-shuffle-line' icon, 15 order_no FROM dual) s
  ON (t.CODE = s.code) WHEN NOT MATCHED THEN INSERT (CODE, TITLE, ICON, ORDER_NO) VALUES (s.code, s.title, s.icon, s.order_no);
MERGE INTO APP_MENU t USING (SELECT 'CONSUMIBLES' code, 'Consumibles' title, 'ri-file-list-3-line' icon, 16 order_no FROM dual) s
  ON (t.CODE = s.code) WHEN NOT MATCHED THEN INSERT (CODE, TITLE, ICON, ORDER_NO) VALUES (s.code, s.title, s.icon, s.order_no);
MERGE INTO APP_MENU t USING (SELECT 'CAUSAS_CONSULTA' code, 'Causas de consulta' title, 'ri-list-settings-line' icon, 17 order_no FROM dual) s
  ON (t.CODE = s.code) WHEN NOT MATCHED THEN INSERT (CODE, TITLE, ICON, ORDER_NO) VALUES (s.code, s.title, s.icon, s.order_no);
COMMIT;

-- Verificacion
SELECT ID, CODE, TITLE, ORDER_NO, ACTIVE FROM APP_MENU WHERE CODE IN ('ANTIDOPING_SELECCION','CONSUMIBLES','CAUSAS_CONSULTA') ORDER BY ORDER_NO;
...
SELECT USERNAME, PASSWORD_HASH FROM APP_SEC_USER WHERE USERNAME = '68958027838';


-- =====================================================================================
-- PASO 6 de 9  —  app_domain/app-fs-file.sql
-- APP_FS_FILE si no existiera (el creador autoritativo es 00_init; este es el respaldo).
-- =====================================================================================
-- =============================================================================
...
-- Reconcilia columnas de APP_FS_FILE si la tabla ya existia con otra forma
...
-- Importador de Excel: APP_IMPORT_LOTE y APP_IMPORT_FILA (staging) + indices.
-- =====================================================================================
-- =============================================================================
...
-- MED_TAG (EAV del historico), MED_TAG_MIG_LOG y la funcion FN_MED_TAG_GROUP que
--    clasifica los 137 tipos. Destino del ETL de tags.
-- =====================================================================================
-- =============================================================================
...
-- modulos; aqui solo importan los nuestros):
SELECT object_name, object_type, status FROM user_objects
 WHERE status <> 'VALID' AND (object_name LIKE 'APP\_%' ESCAPE '\'
        OR object_name LIKE 'MED\_%' ESCAPE '\' OR object_name = 'FN_MED_TAG_GROUP')
 ORDER BY object_type, object_name;

-- Semillas: roles y menus que dejo 01_rbac_local.sql / 03_menu_examenes.sql
SELECT CODE, NAME, ACTIVE FROM APP_SEC_ROLE ORDER BY ID;
SELECT ID, CODE, TITLE, ORDER_NO FROM APP_MENU ORDER BY ORDER_NO;

-- La funcion de clasificacion del ETL de tags debe estar VALID:
SELECT object_name, status FROM user_objects WHERE object_name = 'FN_MED_TAG_GROUP';
